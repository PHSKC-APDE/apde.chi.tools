...
chi_qa_tro <- function(chi_est = NULL, chi_meta = NULL, acs = F, ignore_trends = T, verbose = FALSE){
  status <- 1
  ...
  if(verbose){
    message("Confirm that variables are of the proper class")
  }
  if(verbose){
  ...
  if(verbose){
    message("Confirm that critical columns are not missing any values")
  }
  ...
  ## Check that time_trends are always provided when tab=="trends" ----
        if(ignore_trends == F){
          if(nrow(chi_est[tab == "trends" & is.na(time_trends)]) > 0 ){
            if(verbose) {
              message(glue::glue("There is at least one row where tab=='trends' & where 'time_trends' is missing.
                            Please fill in the missing value before rerunning chi_qa()"))
            } else {
              return(0)
            }
