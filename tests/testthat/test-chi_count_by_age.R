testthat::test_that("issues zip code warning when zip codes not declared in expeted chi variables are present", {
  ...
  analysis_set <- data.table::data.table(cat1 = c("Zip code","Regions"), cat1_varname = c("zipcode","chi_geo_region"), '_kingcounty' = NA, '_wastate' = NA, demgroups = "x", crosstabs = NA, trends = "x", set = 1, set_indicator_keys = c("malignant_neoplasm", "malignant_neoplasm") )
