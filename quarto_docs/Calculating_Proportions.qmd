...
# Remove crosstabs of big cities with HRAs or Regions, `%in% TRUE` allows rows where values are NA to be kept 
myinstructions <- myinstructions[!(cat1_varname == 'bigcities' & 
                        cat2_varname %in% c('hra20_name', 'chi_geo_region')) %in% TRUE] 

# Remove crosstabs of HRAs or Regions with big cities
myinstructions <- myinstructions[!(cat2_varname == 'bigcities' & 
                                     cat1_varname %in% c('hra20_name', 'chi_geo_region')) %in% TRUE] 
```
...
myestimates[ ,
  cat1 := fifelse(
    ...
    paste0("Birthing person's ", sub("^([A-Za-z]+)", "\\L\\1", cat1, perl = TRUE)),
    cat1
    )
]
...
myestimates[ ,
  cat2 := fifelse(
    ...
    paste0("Birthing person's ", sub("^([A-Za-z]+)", "\\L\\1", cat2, perl = TRUE)),
    cat2
    )
]
```
...
Below is an example of how you would do this. Note that, in this case, the differences show are not real (they are synthetic) and are provied for purely illustrative purposes.
...
olddt[rel_rows,
      result := cap01(result * (1 + sample(c(-1,1), .N, TRUE) * runif(.N, 0.10, 0.15)))]

# Recalcualte numerator, se, CI, rse
changed_rows <- c(abs_rows, rel_rows)

olddt[changed_rows, numerator := round(result * denominator)]
olddt[changed_rows, se := sqrt(result * (1 - result) / denominator)]
olddt[changed_rows,
      `:=`(lower_bound = cap01(result - 1.96 * se),
           upper_bound = cap01(result + 1.96 * se),
           rse         = se / pmax(result, 1e-8))]
