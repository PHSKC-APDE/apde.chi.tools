...
The CHI standards are documented in [SharePoint \> Community Health Indicators](https://kc1.sharepoint.com/teams/DPH-CHI/)  \> CHI-Vizes \> CHI-Standards-TableauReady Output.xlsx. We'll follow those standards throughout our analysis.
...
As stated in the introduction, for this vignette we will focus on the adolescent birth rate (`teen1517`). This is because it is the only true rate, with a population denominator, among all of the birth indicators.
...
#| warning: false
#| message: false
myrates[ , cat1 := fifelse(
  cat1 %chin% c("Ethnicity", "Race", "Race/ethnicity"),
  paste0("Birthing person's ", tolower(cat1)),
  cat1
  )
]

myrates[ ,cat2 := fifelse(
  cat2 %chin% c("Ethnicity", "Race", "Race/ethnicity"),
...
set.seed(98104) 
olddt <- copy(myrates)[year == '2019-2023']
olddt <- olddt[(cat1_group == 'Auburn city' & is.na(cat2)) |
                 (cat1_group == 'Hispanic' & is.na(cat2)) |
                 (cat1_group == 'South' & is.na(cat2)) ]

original_rates <- fsetdiff(myrates, olddt)

olddt[, result := fcase(cat1_group == 'Auburn city', result = 2.1, 
                        cat1_group == 'Hispanic', result = 3.4, 
                        cat1_group == 'South', result = 2.7)]

# Recalculate numerator, se, CI, rse
olddt[, numerator := round(result * denominator / 100000)]
olddt[, se := sqrt(numerator) / denominator * 100000]
olddt[, `:=`(lower_bound = round(pmax(0, result - 1.96 * se), 1),
           upper_bound = round(result + 1.96 * se, 1),
           rse         = round(se / pmax(result, 1e-8), 2) )]

olddt[, year := paste0(latest_year-5, '-', latest_year-1)]

rates_old <- rbind(olddt, original_rates)
```

```{r compare-for-real}
#| include: false

# Compare old 5-year rates to synthetic 5-year rates
mycomparison <- chi_compare_estimates(OLD = rates_old,
                                      NEW = myrates,
                                      OLD.year = paste0(latest_year-5, '-', latest_year-1),
                                      NEW.year = paste0(latest_year-4, '-', latest_year),
...
Let's examine two rows from `mycomparison` where `notable == 1`, meaning there is a substantial difference:
...
Remember that this workflow is specifically for rate calculations. For calculations of proportions or means, please refer to the [dedicated vignette](https://github.com/PHSKC-APDE/apde.chi.tools/wiki/Calculating_Proportions).
