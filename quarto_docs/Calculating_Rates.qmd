...
Note that the `povgeo` parameter is dependent upon how the data source defines the `'pov200grp'` indicator in the ETL process. In birth data it is defined by blocks and some other data it is defined by ZIP codes.
...
We can now pass `mypop.instructions` to `chi_get_proper_pop()` to download and structure population data.
...
Since `rads::age_standardize()` is not specific to CHI, you will have to massage the estimates a bit to align them with CHI standards. If you are curious about the SE and RSE approximations below, you are encouraged to examine `?rads::age_standardize`.
