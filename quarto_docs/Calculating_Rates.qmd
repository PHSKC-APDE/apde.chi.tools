...
The `apde.chi.tools` package provides tools for a standardized workflow for preparing King County Community Health Indicators (CHI) estimates. This vignette demonstrates how to use the package's core functions to generate estimates of ***rates that use population denominators***. If you need to calculate prevalences or proportions, please refer to the [separate vignette](https://github.com/PHSKC-APDE/apde.chi.tools/wiki/Calculating_Prevalences) for that purpose.

For the sake of simplicity, we'll walk through the analysis pipeline for calculating the adolescent birth rate as an example. However, estimation of the CHARS and death rates follows the same basic process, albeit with some additional complexity that can be observed in the most recent code in the [CHI repository](https://github.com/PHSKC-APDE/chi).
...
As its name implies, `chi_count_by_age()` creates a detailed breakdown of counts by age for CHI data analysis. Summing the data by age is critical for performing age standardized analyses after merging with population denominators. Like [`chi_calc()`](https://github.com/PHSKC-APDE/apde.chi.tools/wiki/Calculating_Prevalences), which is used for proportions and means, `chi_count_by_age()` processes data according to the instructions created by `chi_generate_tro_shell()` and handles demographic groupings with special treatment for race and ethnicity variables.

::: callout-note
## What's going on under the hood?

The chi_count_by_age()`function shares many validation steps with`chi_calc()`, including checking inputs against`rads.data::misc_chi_byvars`to ensure they follow CHI encoding standards. After validation, the function uses`rads::calc()``` to generate counts by single-year age values (0-100).``Like ```chi_calc()`,`chi_count_by_age()`applies special handling for race and ethnicity variables, particularly the relationship between race3 and race3_hispanic, to ensure consistent representation in output data`.`A key feature of`chi_count_by_age()\`is its handling of missing combinations. When no data exists for a specific demographic-age group combination (e.g., a particular racial group at age 97), the function creates a complete reference table using Cartesian products to ensure all possible combinations are represented with counts of zero rather than being omitted entirely. This comprehensive approach enables proper age standardization in downstream analyses.

The demographic grouping standards in\`rads.data::misc_chi_byvarscan be traced back to [SharePoint \> Community Health Indicators \> CHI-Vizes](https://kc1.sharepoint.com/teams/DPH-CommunityHealthIndicators/CHIVizes/Forms/AllItems.aspx) \> CHI-Standards-TableauReady Output.xlsx. However, directly referencing a SharePoint file from within an R package creates a fragile dependency on external, user-specific infrastructure that can break portability, reproducibility, and automation.
:::
...
Here are a few random rows from `mycounts` that will show the structure and contents of the `chi_counts_by_age()` output:
...
Pass `mycounts` to `chi_generate_instructions_pop()` to create instructions to pull corresponding population data.

```{r pop_instructions}
#| warning: false
#| message: false
mypop.instructions <- chi_generate_instructions_pop(mycount.data = mycounts, 
                                                  povgeo = 'blk') # pov200grp is defined by blocks in birth data and by ZIP in other data
```
...
Pass `mypop.instructions` to `chi_get_proper_pop()` to get structured population data from our servers.

```{r pop_pull}
#| warning: false
#| message: false
mypop <- chi_get_proper_pop(pop.template = mypop.instructions, 
                            pop.genders = 'f', # limit to female because women are the denominator
                            pop.ages = 15:17,  # limit to ages the define adolescent births
                            is_chars = FALSE)  # only set to TRUE if you need population denominators for CHARS
```

Now let's view some of the resulting population table:

```{r pop_pull_view}
#| echo: false
pretty_kable(
  print(mypop[cat1_varname == 'bigcities' & cat2_group == 'Hispanic' & chi_age == 15][1:8])
)
```

### Tidy `chi_get_proper_pop()` output

Race and ethnicity `cat1` and `cat2` values in `mypop` need to be brought into alignment with the values in the `mycounts`.
...
Now that we've completed the steps that use parallel processing, it's a good idea to set `future::plan(sequential)`. This returns R to its normal, single-threaded mode and prevent surprises in subsequent code.
...
# Need variable named 'age' for rads::age_standardize
...
myrates <- rads::age_standardize(ph.data = mycombo,
                                 ...
                                 conf.level = 0.90,
                                 group_by = c("indicator_key", "year", "tab", "cat1", "cat1_group", "cat1_varname", "cat2", "cat2_group", "cat2_varname"))
```
...
myrates[, chi := 1]
myrates[, source_date := max(mydata$creation_date)]
myrates[, run_date := as.Date(Sys.Date(), "%Y%m%d")]

# Use CRUDE for rates, result & CI because they are for an age stratum, not all ages
# CHI standard for rates is to round result to 1 decimal place, SE to 2, and RSE to 3
myrates[, result := round2(crude.rate, 1)]
myrates[, lower_bound := round2(crude.lci, 1)]
myrates[, upper_bound := round2(crude.uci, 1)]
myrates[count != 0, se := round2(crude.rate / sqrt(count), 2)] # just an approximation because no real corresponding SE for the calculated CI
myrates[count == 0 & (!is.na(pop) & pop != 0), se := 0] # if count is zero but there is a denominator population, the real variance is zero
myrates[count != 0, rse := round2(100 / sqrt(count), 3)] # just an approximation because no real corresponding RSE for the calculated CI

myrates[, numerator := count]
myrates[, denominator := round2(pop)]

# Apply primary and secondary suppression
myrates <- apde.chi.tools::chi_suppress_results(ph.data = myrates,
...
After calculation, we need to perform quality assurance checks to ensure our estimates and metadata conform to CHI standards. `chi_qa_tro()` checks whether the CHI estimates and metadata are properly formatted, complete, and compliant with required standards, ensuring that column names, values, and data types meet specified criteria. The function returns 1 for pass and 0 for failures to meet CHI standards. The function also provides warnings when esstimates have unexpected patterns that do not not necessarily violate CHI standards.
...
Remember that this workflow is specifically for prevalence/proportion calculations. For rate calculations that use population denominators, please refer to the dedicated [dedicated rate calculation vignette](https://github.com/PHSKC-APDE/apde.chi.tools/wiki/Calculating_Rates).
